...
def test_disabled_allows(tmp_path):
    from fettle.deploy_gate import run_check
...
def test_deploy_without_tests_advisory(tmp_path):
    ...
    with patch("fettle.deploy_gate._check_tests_ran", return_value=False):
...
def test_deploy_with_all_preconditions_allows(tmp_path):
    ...
    with (patch("fettle.deploy_gate._check_tests_ran", return_value=True),
          patch("fettle.deploy_gate._check_debug_flags", return_value=[])):
        result = run_check(ctx)
