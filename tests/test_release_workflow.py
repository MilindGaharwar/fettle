...
def test_repository_actions_use_immutable_node24_pins():
    action_files = [ROOT / "action.yml", *(ROOT / ".github" / "workflows").glob("*.yml")]
    contents = "\n".join(path.read_text(encoding="utf-8") for path in action_files)

    for action, sha in NODE24_ACTIONS.items():
        references = [line for line in contents.splitlines() if f"uses: {action}@" in line]
        assert references
        assert all(f"uses: {action}@{sha}" in line for line in references)
...
def test_release_smokes_complete_default_install_and_all_compatibility_extra():
    workflow = _workflow()
    build = workflow[workflow.index("  build:"):workflow.index("  publish:")]

    assert "/tmp/smoke/bin/pip install --quiet dist/*.whl" in build
    assert "/tmp/smoke/bin/fettle demo" in build
    assert "import playwright, pytest, yaml" in build
    assert "bindir = Path('/tmp/smoke/bin')" in build
    assert "('fettle', 'mutmut', 'ruff', 'semgrep')" in build
