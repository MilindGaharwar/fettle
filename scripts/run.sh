...
_ok() {  # is $1 a python >= 3.10?
    [ -n "$1" ] && [ -x "$1" ] && "$1" -c 'import sys; sys.exit(0 if sys.version_info >= (3, 10) else 1)' 2>/dev/null
}
...
if ! _ok "$PYTHON"; then
    PYTHON="$(ls -d "$HOME"/.local/share/uv/python/cpython-3.1[0-9]*/bin/python3 2>/dev/null | sort -V | tail -1 || true)"
fi

if ! _ok "$PYTHON"; then
    echo "fettle: no Python >= 3.10 found (tried FETTLE_PYTHON, python3, python3.10-3.13, uv). Set FETTLE_PYTHON." >&2
