...
def init_pre_commit(repo_root: Path, dry_run: bool) -> list[Step]:
    ...
    if config_path.exists():
        ...
        expected = f"rev: v{__version__}"
        if "github.com/MilindGaharwar/fettle" in text and expected not in text:
            steps.append(Step(
                "pre-commit-config", "action",
                f"Fettle revision is stale — update it to v{__version__}",
            ))
        else:
    ...
    binary = resolve_tool("pre-commit")
    if not binary:
